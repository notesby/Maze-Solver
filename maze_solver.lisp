...
(defun get-memory-state (id nodes)
	"Busca un estado en una lista de nodos que sirve como memoria de intentos previos"
	(cond ((null nodes) nil)
			((eql (first (first nodes)) id) (first nodes))
			(T (get-memory-state id (rest nodes)))) )
;(or (= #b1010 (logior #b1010 #b0001)) (= #b0101 (logior #b0101 #b0001))) 
(defun valid-operator (op state) 
	...
	(let ( (mask (get-mask op)) (walls (get-cell-walls (first state) (second state))))  
		(cond ( (not (or (= walls 16) (= walls 17))) (not (= walls (logior walls mask))))
			( (= walls 16) 
				(let ((previous-state (second (get-memory-state (third (get-memory-state *current-ancestor* *memory*)) *memory*) )))
					;(print (List previous-state state op (or (equal op '(0 1)) (equal op '(0 -1))) (or (< (second previous-state) (second state)) (> (second previous-state) (second state))))) 
					(cond ((< (second previous-state) (second state)) (equal op '(0 1)))
						((> (second previous-state) (second state)) (equal op '(0 -1)))
						((> (first previous-state) (first state)) (equal op '(-1 0)))
						((< (first previous-state) (first state)) (equal op '(1 0))) ) ))
			( (= walls 17) 
				(let ((previous-state (second (get-memory-state (third (get-memory-state *current-ancestor* *memory*)) *memory*) ))) 
					(cond ((> (first previous-state) (first state)) (equal op '(-1 0)))
						((< (first previous-state) (first state)) (equal op '(1 0)))
						((< (second previous-state) (second state)) (equal op '(0 1)))
						((> (second previous-state) (second state)) (equal op '(0 -1))) )) )
			( T  nil) )))
;;======================================================================= or (= #b1010 (logior #b1010 mask)) (= #b0101 (logior #b0101 mask)) 
;;  VALID-STATE [state]
   			...
   			(if (and (and (>= new-row 0) (< new-row (get-maze-rows)) ) 
   				...
   				(let* ((new-walls (get-cell-walls new-row new-col))
   					...
   					(or (or (= 0 (first op)) (= 0 (second op)))
   						(and (and (not (= new-walls 16)) (not (= new-walls 17))) ;; verifica que no salte en diagonal a este tipo de celdas
   							(not (= (get-cell-walls (first cur-state) (second new-state)) 16)) 
   							(not (= (get-cell-walls (first new-state) (second cur-state)) 16))
   							(not (= (get-cell-walls (first cur-state) (second new-state)) 17))
   							(not (= (get-cell-walls (first new-state) (second cur-state)) 17)) 
   							(not (= new-walls (logior new-walls inv-mask))) ;;verifica que si se puede pasar, tmb se puede regresar
...
;;=======================================================================
(defun remember-state? (state op memory)
	"Busca un estado en una lista de nodos que sirve como memoria de intentos previos"
	(let ((walls (get-cell-walls (first state) (second state))) ) 
		(cond ((null memory) nil )
			((and (equal (second (first memory)) state) 
				(or (not (or (= walls 16) (= walls 17))) (equal (fourth (first memory)) (second op))) ) T)
			(T (remember-state? state op (rest memory))) )) )
...
(defun filter-memories (child-states)
    ...
    (cond ((null child-states) nil)
    		((remember-state? (first (first child-states)) (second (first child-states)) *memory*) 
...
(defun printWall () (print *goal*) (loop for i from 0 to (- (get-maze-rows) 1)
						do ( loop for j from 0 to (- (get-maze-cols) 1)
								do (format t "maze[~A][~A]= ~A ~%" i j (get-cell-walls i j) ))))
